...
def test_original(model, testloader, device='cuda'):
    ...
    print(len(testloader), TP, FP, FN, TN)
    # print('Testing Accuracy: {:.3f}'.format(accuracy / len(testloader)))
...
if __name__ == "__main__":
    ...
    if args.output_var == 2:
        ...
        if not independent:
            ...
            model.load_state_dict(torch.load(normalCLFPath, map_location=torch.device('cuda')))
            ...
            # fetch model saved state
            model.load_state_dict(torch.load(covidCLFPath, map_location=torch.device('cuda')))
